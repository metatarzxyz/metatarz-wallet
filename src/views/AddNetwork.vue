...
const onAddNetwork = async () => {
  ...
  // Fix: Move this URL validation outside of the name length check
  try {
    new URL(rpc.value);
  } catch {
  ...
  let networks = allNetworks[0] as Networks;
  const selectedNetwork = allNetworks[1] as Network;

  // Fix: Ensure networks is an object, create empty one if null/undefined
  ...
  // Fix: Check if network already exists (for add mode, not edit)
  if (!isEdit && networks[chainId.value]) {
    alertMsg.value = "Network already exists.";
    alertOpen.value = true;
    return;
  }
  
  // Fix: Handle selected network logic properly
  if (Object.keys(networks).length === 0 || selectedNetwork?.chainId === chainId.value) {
    p1 = saveSelectedNetwork(network);
  }
  
  // Fix: Create a new object to avoid reference issues
  const updatedNetworks = { ...networks };
  updatedNetworks[chainId.value] = network;
  
  const p2 = replaceNetworks(updatedNetworks);
